...
from modules.sql import read_client, update_or_create_client
...
def get_client_id(update):
    client_id = read_client(update.effective_user.id)
    if not client_id:
        client_id = 0
        update_or_create_client(update.effective_user.id, client_id)
    return client_id
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Hi!", reply_markup=ReplyKeyboardMarkup([[KeyboardButton('Deezer' if client_id else 'Qobuz')]], resize_keyboard=True))


async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send a message when the command /help is issued."""
    client_id = get_client_id(update)
    await update.message.reply_text("Help!", reply_markup=ReplyKeyboardMarkup([[KeyboardButton('Deezer' if client_id else 'Qobuz')]], resize_keyboard=True))
...
async def downloading(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message.text == 'Qobuz' or update.message.text == 'Deezer':
        client_id = int(not client_id)
        update_or_create_client(update.effective_user.id, client_id)
        await update.message.reply_text(f"Changed client to {'Deezer' if client_id else 'Qobuz'}", reply_markup=ReplyKeyboardMarkup([[KeyboardButton('Deezer' if client_id else 'Qobuz')]], resize_keyboard=True))
        return
    ...
    await update.message.reply_photo(photo=meta.cover_urls['original'], caption=text, reply_markup=ReplyKeyboardMarkup([[KeyboardButton('Deezer' if client_id else 'Qobuz')]], resize_keyboard=True))

    downloading_message = await update.message.reply_text('Downloading...')
